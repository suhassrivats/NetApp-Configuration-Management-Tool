...
extern char user[] ;  
extern char passwd[];    	
int smresync(int argc, char **argv)
{
        na_server_t*    s;
        na_elem_t*      out;
        char            err[256];
	...
	if (kbrate <= 0 && !strcmp (snapshot, ""))
		out = na_server_invoke(s, "snapmirror-resync", 								"source-location", srel,
					"destination-location", drel,
					NULL);
	else if (kbrate > 0 && !strcmp (snapshot, ""))
		out = na_server_invoke (s, "snapmirror-resync", 							"source-location", srel,							"destination-location", drel,
					"max-transfer-rate", kbstr, NULL						);
